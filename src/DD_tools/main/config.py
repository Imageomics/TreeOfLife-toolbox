...
TEMPLATE_PATH = {"tools": "config_templates/tools.yaml"}
...
class Config:
    ...
    @classmethod
    def from_path(cls, path: str, config_type: Literal["tools"] = "tools") -> "Config":
    ...
    @staticmethod
    def __check_config(
            cfg: Dict[str, str | int | bool | Dict[str, Any]],
            config_type: Literal["downloader", "tools"],
    ) -> bool:
        config_template_path = os.path.join(
            Path(__file__).parent.absolute(), TEMPLATE_PATH[config_type]
        )
        if not os.path.exists(config_template_path):
            raise FileNotFoundError(
                f"Config template not found, can't check it {config_template_path}"
            )
    ...
    def __load_folder_structure(
        ...
        file_structure.update(
            {
                key: os.path.join(cfg["path_to_output_folder"], value)
                for key, value in cfg["output_structure"].items()
            }
        )
